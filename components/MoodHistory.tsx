...
export default function MoodHistory({ entries, onDeleteEntry, onQuickLog }: MoodHistoryProps) {
  ...
  return (
    ...
    >
      ...
      {/* Main List / Content (flex-1 flex flex-col justify-center) */}
      <div className="flex-grow flex flex-col justify-center">
        {entries.length === 0 ? (
          <div
            className="text-center rounded-[20px] border border-dashed py-8 px-4 flex-grow flex flex-col items-center justify-center"
            style={{
              backgroundColor: "var(--background)",
              borderColor: "var(--card-border)",
            }}
          >
            ...
            <p className="text-sm font-bold mb-1 mt-3" style={{ color: "var(--text-primary)" }}>
              No entries logged yet
            </p>
            <p className="text-xs max-w-xs mx-auto leading-relaxed pb-4" style={{ color: "var(--text-secondary)" }}>
            ...
            <div className="flex flex-wrap gap-2 justify-center mt-2">
              <button
                type="button"
                onClick={() => handleQuickLog(getTodayDateString())}
                className="px-4 py-2 rounded-xl text-xs font-black uppercase tracking-wider transition-all duration-300 active:scale-[0.98] border"
                style={{
              ...
              <button
                type="button"
                onClick={() => handleQuickLog(getYesterdayDateString())}
                className="px-4 py-2 rounded-xl text-xs font-black uppercase tracking-wider transition-all duration-300 active:scale-[0.98] border"
                style={{
                  backgroundColor: "var(--background)",
          ...
          <div className="text-center py-12 text-sm font-bold flex-grow flex items-center justify-center" style={{ color: "var(--text-secondary)" }}>
            No entries matched &quot;{searchTerm}&quot;
          </div>
        ) : (
          <div className="space-y-4 max-h-[420px] overflow-y-auto pr-1 scrollbar-thin scrollbar-thumb-rounded flex-grow">
            {filteredEntries.map((entry) => {
              ...
              return (
                <div
                  key={entry.id}
                  className="flex items-start justify-between gap-4 p-6 rounded-[20px] border transition-all duration-200 hover:shadow-md bg-transparent"
                  style={{
                    borderColor: "var(--card-border)",
                    boxShadow: "var(--card-shadow)",
                    borderLeftWidth: "4px",
